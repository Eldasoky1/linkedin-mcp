...
async function addPosition(
  ...
  // Step 5: Click "Back" to return to edit intro
  const backBtn = page.locator("button:has-text('Back')").first();
  const backVisible = await backBtn.isVisible().catch(() => false);
  if (backVisible) {
    await backBtn.click();
    await page.waitForTimeout(3000);
    console.log("  [OK] Back clicked, URL:", page.url());
  }

  // Step 6: Click the main "Save" on edit intro page to publish all changes
  ...
  if (mainSaveVisible) {
    ...
    await page.waitForTimeout(3000);
    console.log("  [OK] Main Save on edit intro clicked");
  } else {
    // Maybe still on position form, try again
    console.log("  [!] Main Save not visible yet");
  }
}
